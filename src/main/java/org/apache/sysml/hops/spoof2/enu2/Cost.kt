...
data class SCost(
    ...
    companion object {
        fun costSPlan(roots: List<SNode>, costMemo: MutableMap<Id, Double>) {
            for (root in roots)
                costSPlan(root, costMemo)
        }

        private fun costSPlan(node: SNode, costMemo: MutableMap<Id, Double>) {
            if( node.id in costMemo )
                return
            val nextNodes: List<SNode> = when( node ) {
                is SNodeAggregate -> costAgg(node, costMemo)
                is SNodeNary -> costNary(node, costMemo)
                else -> node.inputs
            }
            nextNodes.forEach { costSPlan(it, costMemo) }
        }

        private fun costAgg(agg: SNodeAggregate, costMemo: MutableMap<Id, Double>): List<SNode> {
            if( agg.op != Hop.AggOp.SUM) // only cost SUM aggregates
                return listOf(agg.input)
        ...
        private fun costNary(nary: SNodeNary, costMemo: MutableMap<Id, Double>): List<SNode> {
