...
//                assert(!(aggInput is SNodeAggregate && aggInput.op == Hop.AggOp.SUM)) // RewriteAggregateElim() should handle this case
            ...
            if (!ThrownError)
                Spoof2Compiler.LOG.error(Explain.explainSPlan(listOf(agg), true))
            ThrownError = true
            throw e
        }
    }
        ...
        try {
            ...
            if (nary.inputs.size == 2) {
                ...
                // transpose vector if it does not match the correct dimension
                    ...
                    } else throw SNodeException(nary, "attribute name of vector does not match either attribute name of matrix in vector-expansion element-wise multiply; mis is $mis")
                }
            }
            ...
            if (!ThrownError)
                Spoof2Compiler.LOG.error(Explain.explainSPlan(listOf(nary), true))
            ThrownError = true
            throw e
        }
    }
...
//                    hop
            ...
            if (!ThrownError)
                Spoof2Compiler.LOG.error(Explain.explainSPlan(listOf(current), true))
            ThrownError = true
            throw e
        }
    }
