...
do 
   #generate input data
   if [ "${genData}" == 1 ]; then
      ./sparkDML.sh --config SystemML-config_fused_gen.xml -f ./dml/DatagenClass.dml --args ${num_rows} ${num_cols} 5 5 spoof/w2_${num_rows} spoof/X2_${num_rows} spoof/y2_${num_rows} 1 0 ${sparsity} binary 0
   fi
